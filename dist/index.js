...
const CONFIG_PATH = (0, path_1.join)((0, os_1.homedir)(), ".midea-ac.json");
const isAgent = !process.stdout.isTTY;
function loadConfig() {
    if (!(0, fs_1.existsSync)(CONFIG_PATH)) {
        respondError("midea-ac", `No config found at ${CONFIG_PATH}`, "Run 'midea-ac setup' to discover and configure your device");
        process.exit(1);
    }
    try {
        return JSON.parse((0, fs_1.readFileSync)(CONFIG_PATH, "utf8"));
    }
    catch {
...
function runLocal(cmd) {
    return (0, child_process_1.execSync)(cmd, { encoding: "utf8", timeout: 15000 });
}
function getMsmartPath() {
    const candidates = [
        "msmart-ng",
        `${(0, os_1.homedir)()}/Library/Python/3.9/bin/msmart-ng`,
        `${(0, os_1.homedir)()}/Library/Python/3.11/bin/msmart-ng`,
        `${(0, os_1.homedir)()}/Library/Python/3.12/bin/msmart-ng`,
        "/usr/local/bin/msmart-ng",
        "/usr/bin/msmart-ng",
    ];
    for (const p of candidates) {
        try {
            (0, child_process_1.execSync)(`${p} --version 2>/dev/null`, { encoding: "utf8" });
            return p;
        }
        catch {
            // not found, try next
    ...
    respondError("midea-ac", "msmart-ng not found", "Install it with: pip install msmart-ng");
    process.exit(1);
}
function parseQueryOutput(raw) {
    const get = (key) => {
...
function formatMode(mode) {
    ...
    if (m) {
        const map = { AUTO: "Auto", COOL: "Cool", DRY: "Dry", HEAT: "Heat", FAN_ONLY: "Fan" };
        return map[m[1]] ?? m[1];
    }
    return mode;
}
function formatFan(fan) {
    ...
    if (m) {
        const map = { SILENT: "Silent", LOW: "Low", MEDIUM: "Medium", HIGH: "High", AUTO: "Auto", MAX: "Max" };
        return map[m[1]] ?? m[1];
    }
    return fan;
}
function respond(command, result, next_actions = []) {
    if (isAgent) {
        console.log(JSON.stringify({ ok: true, command, result, next_actions }));
    }
    return result;
}
function respondError(command, message, fix) {
    if (isAgent) {
...
program
    ...
    if (!ipMatch || !idMatch || !keyMatch || !tokenMatch) {
        respondError("midea-ac setup", "No devices found on the local network", "Make sure your AC is powered on, on WiFi, and your machine is on the same network");
        return;
    }
    const config = {
        ip: ipMatch[1],
...
program
    ...
    if (result && !isAgent) {
        ...
        if (normalized.filter_alert)
            console.log("⚠️  Filter alert: clean the filter");
    }
});
program
    .command("on")
...
program
    ...
    if (!isAgent)
        console.log(`Cool mode set to ${f}°F.`);
});
program
    .command("heat <temp>")
    ...
    if (!isAgent)
        console.log(`Heat mode set to ${f}°F.`);
});
program
    .command("fan <speed>")
    .description("Set fan speed: auto | low | medium | high | silent")
    .action((speed) => {
    const speedMap = { silent: 20, low: 40, medium: 60, high: 80, auto: 102 };
...
program
    ...
    if (!isAgent)
        console.log(`Eco mode ${on ? "on" : "off"}.`);
});
program
